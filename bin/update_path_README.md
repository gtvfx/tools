...
Automatically discovers all git repositories in the workspace and adds their `bin` subdirectories to the PATH environment variable.

## Features

- **Recursive Search**: Finds all git repositories (directories with `.git`) in the workspace
- **Smart Detection**: Only adds `bin` subdirectories that exist
- **Duplicate Prevention**: Checks existing PATH and skips already-added paths
...
```powershell
# Update PATH for current session only
.\update_path.ps1

# Update PATH permanently (User-level)
.\update_path.ps1 -Permanent

# Show verbose output with all paths
.\update_path.ps1 -Verbose

# Combine options
.\update_path.ps1 -Permanent -Verbose
```
...
- **`-Verbose`**: Shows detailed output including current PATH entries (filtered to workspace), all discovered repositories, and new PATH additions.
- **`-WorkspaceRoot`**: Specify a custom workspace root (default: parent of tools directory)

Note: The `update_path.bat` wrapper accepts friendly forms of these flags (e.g., `permanent`, `verbose` — case-insensitive) without a leading dash, and recognizes `-h`, `--help`, `help`, or `?` to display usage and exit.
...
Found 13 git repositories
  + Adding: \\THO_CLOUD\gavyn\repo\python_packages\bin
  + Adding: \\THO_CLOUD\gavyn\repo\tools\bin
  + Adding: \\THO_CLOUD\gavyn\repo\robinhood\bin
  + Adding: \\THO_CLOUD\gavyn\repo\coinbase\bin

Updating PATH...
[OK] PATH updated for current session

Added 4 new path(s) to PATH
...
3. Checks each repository for a `bin` subdirectory
4. Compares found paths with existing PATH entries
5. Adds only new, unique paths to PATH
6. Updates the environment variable (current session or permanent)

## Related

- See [update_pythonpath](update_pythonpath_README.md) for the companion script that scans repositories for `py/` directories and adds them to your PYTHONPATH.
...
- **Execution Policy**: If you get an execution policy error, run: `powershell -ExecutionPolicy Bypass -File .\update_path.ps1`
...
- Check that `bin` subdirectory exists in each repository
